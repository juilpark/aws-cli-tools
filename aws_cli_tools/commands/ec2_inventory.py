...
from ..constants import (
    ...
    EC2_INVENTORY_OUTPUT_DIR,
)
from ..ec2_inventory import collect_ec2_instance_inventory, write_inventory_csv
...
def ec2_inventory(
    output_dir: Path = typer.Option(
        ...
        file_okay=False,
        dir_okay=True,
        help="Directory where the timestamped EC2 inventory CSV will be saved",
    ),
    ...
    """
    List EC2 instances in every enabled region and save an RI/SP review CSV.
    """
    try:
        records = collect_ec2_instance_inventory(
            connect_timeout=connect_timeout,
            read_timeout=read_timeout,
            max_attempts=max_attempts,
        )
        output_path = write_inventory_csv(records, output_dir)
        typer.echo(f"Collected {len(records)} EC2 instances.")
